HealthCheck.setup do |config|
  ...
  config.standard_checks = [ 'database', 'migrations', 'custom', 'unicorn' ]

  # Or to exclude one check:
  config.standard_checks -= [ 'emailconf' ]

  # You can set what tests are run with the 'full' or 'all' parameter
  config.full_checks = ['database', 'migrations', 'custom', 'email', 'cache', 'redis', 'resque-redis', 'sidekiq-redis', 's3', 'sidekiq']
